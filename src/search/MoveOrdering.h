...
private:
    ...
    struct KillerMoves {
        ...
        void add(const MoveContent& move) {
            if (move.capture != ChessPieceType::NONE) return;
            
            if (count > 0 && killers[0] == move) return;
        ...
        bool isKiller(const MoveContent& move) const {
            return count > 0 && (killers[0] == move || 
                   (count > 1 && killers[1] == move));
        }
        
        int getKillerScore(const MoveContent& move) const {
            if (count > 0 && killers[0] == move) return FIRST_KILLER_BONUS;
            if (count > 1 && killers[1] == move) return SECOND_KILLER_BONUS;
            return 0;
        }
    };
...
public:
    ...
    void updateCounterMove(const MoveContent& previousMove, const MoveContent& move, ChessPieceColor side) {
        if (previousMove.src >= 0 && previousMove.src < 64 && previousMove.dest >= 0 && previousMove.dest < 64) {
    ...
    void updateCounterMoveHistory(ChessPieceType piece, int from, ChessPieceType prevPiece, 
        ...
        if (piece == ChessPieceType::NONE || prevPiece == ChessPieceType::NONE) return;
        ...
        if (pieceIdx >= 0 && pieceIdx < 6 && prevPieceIdx >= 0 && prevPieceIdx < 6) {
            ...
            if (good) {
                value += delta;
                if (value > HISTORY_MAX) value = HISTORY_MAX;
            } else {
                value -= delta;
                if (value < -HISTORY_MAX) value = -HISTORY_MAX;
            }
    ...
    int scoreMove(const MoveContent& move, const Board& board, int ply, 
        ...
        if (score == 0 && previousMove != nullptr && previousMove->dest >= 0 && previousMove->dest < 64) {
            ChessPieceColor side = board.turn;
            const MoveContent& counter = counterMoves[side == ChessPieceColor::WHITE ? 0 : 1][previousMove->dest];
            if (counter == move) {
        ...
        if (score == 0) {
            ...
            if (previousMove != nullptr && previousMove->piece != ChessPieceType::NONE) {
                ...
                if (pieceIdx >= 0 && pieceIdx < 6 && prevPieceIdx >= 0 && prevPieceIdx < 6 &&
                    ...
                    score += counterMoveHistory[pieceIdx][move.src][prevPieceIdx][previousMove->dest];
                }
            }
        }
