...
void GenerateAllMoves(Board& board, std::vector<MoveContent>& moves) {
    ...
    for (int from = 0; from < 64; from++) {
        if (board.squares[from].piece.PieceType == ChessPieceType::NONE) continue;
        if (board.squares[from].piece.PieceColor != color) continue;
...
int quiescence(Board& board, int alpha, int beta, int depth) {
    ...
    for (const auto& move : moves) {
        ...
        if (!MakeMove(newBoard, move)) continue;
...
int alphaBeta(Board& board, int depth, int alpha, int beta, bool pvNode, 
              bool cutNode, MoveContent* prevMove, bool doNull) {
    ...
    if (!isRoot) {
        ...
        if (ttHit) {
            ...
            if (ttEntry->depth >= depth && !pvNode) {
                if (ttEntry->flag == EXACT) return ttValue;
                if (ttEntry->flag == LOWERBOUND && ttValue >= beta) return ttValue;
                if (ttEntry->flag == UPPERBOUND && ttValue <= alpha) return ttValue;
            }
        }
    }
    ...
    if (!inCheck) {
        ...
        if (!pvNode && doNull && depth >= 3 && staticEval >= beta && 
            ...
            searchInfo.ply++;
            int nullScore = -alphaBeta(nullBoard, depth - R - 1, -beta, -beta + 1, 
                                       false, !cutNode, nullptr, false);
            searchInfo.ply--;
        ...
        if (enhancements.getProbCut().canApply(depth, beta, pvNode, inCheck)) {
            ...
            for (const auto& move : captures) {
                ...
                searchInfo.ply++;
                int score = -alphaBeta(newBoard, depth - 4, -probCutBeta, -probCutBeta + 1,
                                      false, !cutNode, &move, true);
                searchInfo.ply--;
    ...
    for (const auto& move : moves) {
        ...
        if (isQuiet) {
            ...
            if (skipQuiets && !givesCheck) continue;
            
            if (!inCheck && !givesCheck) {
                if (enhancements.getLMP().canPrune(depth, quietMoveNum, improving, 
                                                   inCheck, false, staticEval, beta)) {
                    skipQuiets = true;
                    continue;
                }
                
                if (enhancements.getFutility().canPrune(depth, staticEval + getPieceValue(move.piece),
                                                        alpha, beta, inCheck)) {
                    continue;
                }
            }
        ...
        if (inCheck) {
            ...
            Board testBoard = board;
            searchInfo.ply++;
            int singularScore = alphaBeta(testBoard, singularDepth, singularBeta - 1, 
                                         singularBeta, false, cutNode, prevMove, true);
            searchInfo.ply--;
        ...
        if (moveNum == 1) {
            ...
            score = -alphaBeta(newBoard, newDepth - reduction, -alpha - 1, -alpha, 
                             false, true, &move, true);
            
            if (score > alpha && reduction > 0) {
                score = -alphaBeta(newBoard, newDepth, -alpha - 1, -alpha, 
                                 false, !cutNode, &move, true);
            }
            
            if (score > alpha && score < beta) {
                score = -alphaBeta(newBoard, newDepth, -beta, -alpha, 
                                 true, false, &move, true);
            }
        }
    ...
    TTFlag flag = (bestScore >= beta) ? LOWERBOUND :
                  (bestScore <= originalAlpha) ? UPPERBOUND : EXACT;
    ...
    for (int depth = 1; depth <= maxDepth && !searchInfo.stopped; depth++) {
        ...
        if (depth >= 5) {
            ...
            while (!searchInfo.stopped) {
                score = alphaBeta(board, depth, aspiration.alpha, aspiration.beta, 
...
uint64_t ComputeHash(const Board& board) {
    ...
    for (int i = 0; i < 64; i++) {
        if (board.squares[i].piece.PieceType != ChessPieceType::NONE) {
            hash ^= ((uint64_t)i << 32) | ((uint64_t)board.squares[i].piece.PieceType << 16) | 
