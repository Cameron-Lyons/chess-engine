...
struct Move {
    ...
    Move(int f, int t, std::string_view n = "") 
        : from(f), to(t), notation(n) {}
...
struct ModernPiece {
    ...
    ModernPiece(int t, int c, Position p) 
        : type(t), color(c), pos(p) {}
...
namespace ModernChessUtils {
    ...
    std::string formatMove(const Move& move) {
        return std::string("Move from ") + std::to_string(move.from) + 
    ...
    std::string formatError(ChessError error) {
        switch (error) {
            ...
            case ChessError::NoPieceAtSource: return "No piece at source";
            case ChessError::WrongTurn: return "Wrong turn";
            ...
            case ChessError::InvalidFEN: return "Invalid FEN";
            case ChessError::Timeout: return "Timeout";
