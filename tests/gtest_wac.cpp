...
protected:
    ...
    bool checkMove(const char* fen, int expectedFrom, int expectedTo, int depthLimit,
        ...
        SearchResult result = runSearch(board, depthLimit, timeLimitMs, 1, 0, 1);
...
TEST_F(WACTest, WAC002_QxMate) {
    ...
    SearchResult result = runSearch(board, 8, 5000, 1, 0, 1);
...
TEST_F(WACTest, EndgameConversion) {
    ...
    SearchResult result = runSearch(board, 10, 5000, 1, 0, 1);
    EXPECT_TRUE(result.bestMove.first >= 0);
}
...
TEST_F(WACTest, PawnRace) {
    ...
    SearchResult result = runSearch(board, 12, 5000, 1, 0, 1);
    EXPECT_TRUE(result.bestMove.first >= 0);
}
